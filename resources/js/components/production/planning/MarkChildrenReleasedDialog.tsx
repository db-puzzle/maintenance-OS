...
interface MarkChildrenReleasedDialogProps {
    open: boolean;
...
export function MarkChildrenReleasedDialog({
    open,
    onOpenChange,
    onConfirm,
    parentOrder,
    childrenCount,
}: MarkChildrenReleasedDialogProps) {
    ...
    // Check if a child is not in planned status
    const isNotPlanned = (order: ManufacturingOrder): boolean => {
            ...
            <DialogContent
                ...
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        ...
                        Liberar Ordens Filhas para Produção?
                    </DialogTitle>
                    <DialogDescription asChild>
                        <div className="space-y-3 pt-2">
                            ...
                            <p>
                                Deseja liberar {childrenCount === 1 ? 'a ordem filha' : 'as ordens filhas'} para produção também?
                            </p>
                            <div className="flex items-center gap-2 rounded-md bg-muted/50 p-3">
                                <AlertCircle className="h-4 w-4 text-muted-foreground" />
                                <p className="text-sm text-muted-foreground">
                                    Apenas ordens no status "Planejada" com rotas válidas serão liberadas.
                                </p>
                            </div>
                        </div>
                    </DialogDescription>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {isNotPlanned(child) && (
                                                            <div className="flex items-center gap-1">
                                                                ...
                                                                <span className="text-xs text-orange-600">Status: {child.status}</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
