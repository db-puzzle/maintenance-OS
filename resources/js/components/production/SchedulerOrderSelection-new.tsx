                        ...
                        <div className="flex-1 overflow-hidden flex flex-col p-6">
                            ...
                            <div className="flex-1 overflow-y-auto">
                                {loadingTimeParams ? (
                                    ...
                                    <TimeParameterStatus
                                        orders={timeParameterData.map(data => {
                                            const order = orders.find(o => o.id === data.order_id);
                                            return {
                                                id: data.order_id,
                                                order_number: order?.order_number || `Order ${data.order_id}`,
                                                item: {
                                                    id: order?.item?.id || 0,
                                                    name: order?.item?.name || '',
                                                ...
                                                status: order?.status || '',
                                                has_route: true,
                                                time_parameter_status: (data.time_parameter_status as 'valid' | 'partial' | 'missing') || 'missing',
                                                steps: [],
                                                issues: []
                                            };
                                        })}
                    ...
                    <div className="text-sm text-muted-foreground">
                        ...
                        {currentStep === 2 && (
                            timeParameterData.some(order => order.time_parameter_status !== 'valid') ?
                                'Configure all time parameters to continue' :
