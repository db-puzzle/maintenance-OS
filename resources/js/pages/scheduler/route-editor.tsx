...
import { PlusCircle, Save, Trash2, X, Move, LayoutGrid, ArrowRight, ArrowDown, HelpCircle } from 'lucide-react';
...
const WORKCELL_COLORS = [
  '#e9d8fd', '#feebc8', '#c6f6d5', '#bee3f8', '#fed7d7', 
  '#e2e8f0', '#fbd38d', '#b2f5ea', '#feb2b2', '#fbd5e5',
  '#c4f1f9', '#b2f5ea', '#c3dafe', '#e9d8fd', '#fefcbf'
];
...
export default function RouteEditor({}: Props) {
    ...
    const handleConnectionEnd = (targetNodeId: string) => {
        ...
        if (existingConnection) {
            ...
            setConnectingFrom(null);
            toast.error("Já existe uma conexão entre estes postos de trabalho. Apenas uma conexão é permitida entre quaisquer dois postos de trabalho.");
            return;
        }
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col h-full">
                {/* Toolbar */}
                <div className="flex p-4 bg-gray-100 border-b border-gray-200">
                ...
                <div className="flex flex-1 overflow-hidden">
                    ...
                    <div className="w-64 bg-gray-50 border-r border-gray-200 p-4 overflow-y-auto">
                        <h3 className="font-semibold mb-3">Postos de Trabalho Disponíveis</h3>
                        
                        {/* Workcells padrão */}
                        {AVAILABLE_WORKCELLS.map(workcell => (
                            <div 
                                key={workcell.id}
                                className={`bg-white border border-gray-200 rounded p-3 mb-2 flex items-center cursor-move hover:bg-gray-50 transition-colors ${draggingWorkcell === workcell.id ? 'opacity-50 border-blue-500' : ''}`}
                                draggable="true"
                                onDragStart={(e) => {
                        ...
                        {!showCustomForm ? (
                            <Button
                                variant="outline"
                                className="mt-3 w-full py-5 bg-green-50 hover:bg-green-100 text-green-800 border-green-300"
                            ...
                            <div className="mt-3 p-3 bg-white border border-gray-200 rounded">
                                <h4 className="font-medium mb-2">Novo Posto de Trabalho</h4>
                                <input
                                    type="text"
                                ...
                                <div className="mb-2">
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                        {WORKCELL_COLORS.map((color, index) => (
                                            <div
                                                key={index}
                                                className={`w-6 h-6 rounded-full cursor-pointer border ${newWorkcellColor === color ? 'border-black' : 'border-gray-300'}`}
                                    ...
                                    <div className="mt-4">
                                        ...
                                        <ul className="space-y-2 text-sm text-gray-600">
                                            <li className="flex items-start gap-2">
                                                <div className="bg-blue-100 p-1 rounded text-blue-600 mt-0.5">1</div>
                                                <span>Arraste postos da barra lateral para o quadro</span>
                                            </li>
                                            <li className="flex items-start gap-2">
                                                <div className="bg-blue-100 p-1 rounded text-blue-600 mt-0.5">2</div>
                                                <span>Crie postos personalizados conforme necessário</span>
                                            </li>
                                            <li className="flex items-start gap-2">
                                                <div className="bg-blue-100 p-1 rounded text-blue-600 mt-0.5">3</div>
                                                <span>Arraste postos existentes para reposicioná-los</span>
                                            </li>
                                            <li className="flex items-start gap-2">
                                                <div className="bg-blue-100 p-1 rounded text-blue-600 mt-0.5">4</div>
                                                <span>Arraste a partir do ponto azul para criar conexões</span>
                                            </li>
                                            <li className="flex items-start gap-2">
                                                <div className="bg-blue-100 p-1 rounded text-blue-600 mt-0.5">5</div>
                                                <span>Apenas uma conexão permitida entre quaisquer dois postos</span>
                                            </li>
                                            <li className="flex items-start gap-2">
                                                <div className="bg-blue-100 p-1 rounded text-blue-600 mt-0.5">6</div>
                                                <span>Use Auto Organizar para layouts horizontais ou verticais</span>
                                            </li>
                                            <li className="flex items-start gap-2">
                                                <div className="bg-blue-100 p-1 rounded text-blue-600 mt-0.5">7</div>
                                                <span>As setas em negrito mostram a direção do fluxo</span>
                                            </li>
                                            <li className="flex items-start gap-2">
                                                <div className="bg-blue-100 p-1 rounded text-blue-600 mt-0.5">8</div>
                    ...
                    <div 
                        className={`flex-1 relative bg-gray-50 overflow-auto transition-all duration-200 ${isDraggingOver ? 'bg-blue-50 border-2 border-dashed border-blue-300 scale-[0.99]' : ''}`}
                        ref={boardRef}
                    ...
                    >
                            ...
                            {edges.map(edge => {
                                ...
                                return (
                                    <g key={edge.id}>
                                        ...
                                        >
                                            <div
                                                className="bg-white rounded-full border border-gray-300 flex items-center justify-center w-5 h-5 cursor-pointer hover:bg-red-100"
                        ...
                        {nodes.map(node => (
                            <div
                                key={node.id}
                                className="absolute bg-white border rounded-md shadow-md cursor-move flex flex-col"
                                style={{
                                ...
                                draggable="false"
                            >
                                <div className="flex justify-between items-center p-2 border-b" style={{ backgroundColor: node.color + '30' }} draggable="false">
                                    <span className="font-regular truncate" draggable="false">{node.name}</span>
                                    <Button 
                                        variant="ghost" 
                                        size="icon" 
                                ...
                                <div className="p-2 text-sm text-gray-600 flex flex-col" draggable="false">
                                    <span draggable="false">ID: {node.id}</span>
                                    <span className="text-xs text-gray-500" draggable="false">
                                        Entrada: {edges.filter(e => e.target === node.id).length} / 
                                        Saída: {edges.filter(e => e.source === node.id).length}
                                    </span>
                                </div>
                                
                                {/* Alça de conexão - área ampla clicável com indicador visual */}
                                <div 
                                    className="absolute w-full h-10 cursor-pointer"
                                ...
                                >
                                    ...
                                    <div 
                                        ...
                                        draggable="false"
                                    >
                                        <div className="w-2 h-2 bg-white rounded-full" draggable="false"></div>
                                    </div>
                                </div>
                                ...
                                {connectingFrom && connectingFrom !== node.id && (
                                    <div 
                                        className="absolute inset-0 bg-blue-100 opacity-30 z-30 border-2 border-blue-400"
                                    />
                                )}
                            </div>
                        ))}
                ...
                {showOutput && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white rounded-lg shadow-xl w-2/3 max-h-2/3 flex flex-col">
                            ...
                            <div className="flex-1 overflow-auto p-4">
                                <pre className="bg-gray-50 p-4 rounded border border-gray-200 text-sm overflow-auto">
                                    {jsonOutput}
                                </pre>
                            </div>
