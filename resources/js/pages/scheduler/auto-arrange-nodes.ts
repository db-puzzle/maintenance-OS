...
export function autoArrangeNodes(
  ...
  direction: string = 'horizontal'
    ...
    const rootNodes = nodes
        .filter(node => graph[node.id].inNodes.length === 0)
        .map(node => node.id);
    ...
    while (queue.length > 0) {
        ...
        for (const outNodeId of current.outNodes) {
            ...
            const allInNodesProcessed = outNode.inNodes.every(
                inNodeId => graph[inNodeId].processed || inNodeId === currentId
            );
        ...
        for (const outNodeId of current.outNodes) {
            ...
            if (!outNode.processed) {
                ...
                if (outNode.inNodes.length > 1) {
                    const processedInNodesCount = outNode.inNodes.filter(
                        inNodeId => graph[inNodeId].processed || inNodeId === currentId
                    ).length;
                    ...
                    if (processedInNodesCount === outNode.inNodes.length) {
                        ...
                        let laneTaken = true;
                        while (laneTaken) {
                            laneTaken = Object.values(graph).some(
                                n => n.id !== outNode.id && 
                                     n.level === outNode.level && 
                                     n.lane === outNode.lane
                            );
    ...
    nodes.forEach(node => {
        if (!graph[node.id].processed && 
            graph[node.id].inNodes.length === 0 && 
            graph[node.id].outNodes.length === 0) {
