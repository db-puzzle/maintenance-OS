...
class PlanningController extends Controller
{
    ...
    /**
     * Apply template to a single manufacturing order from planning interface.
     */
    public function applyTemplate(Request $request, ManufacturingOrder $order)
    {
        ...
        if (! in_array($order->status, ['draft', 'planned', 'scheduled'])) {
            return back()->withErrors(['message' => 'Route templates can only be applied to draft, planned, or scheduled orders.']);
        }
        ...
        if ($order->manufacturingRoute && ! ($validated['overwrite'] ?? false)) {
            return back()->withErrors(['message' => 'This order already has a route. Please confirm overwrite in the dialog.']);
        }
        ...
        try {
            ...
            return back()->withErrors(['message' => 'Failed to apply template: ' . $e->getMessage()]);
        }
    }

    /**
     * Bulk apply template to manufacturing orders.
     */
    public function bulkApplyTemplate(Request $request)
    {
        $validated = $request->validate([
            'orderIds' => 'required|array',
            'orderIds.*' => 'exists:manufacturing_orders,id',
            'templateId' => 'required|exists:manufacturing_routes,id',
        ]);
        ...
        foreach ($validated['orderIds'] as $orderId) {
            try {
                ...
                // Apply template
                $this->routeBuilderService->applyTemplateToOrder($order, $template);
        ...
        // TODO: Track template usage in a separate table or field
