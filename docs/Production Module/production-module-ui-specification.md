...
<AppLayout breadcrumbs={breadcrumbs}>
    <ListLayout
        title="Produtos"
        description="Gerencie o catálogo de produtos de manufatura"
        searchPlaceholder="Buscar por número ou nome do produto..."
        searchValue={searchValue}
        onSearchChange={handleSearchChange}
        createRoute="/production/products/create"
        createButtonText="Novo Produto"
    >
        <EntityDataTable
            data={products}
            columns={productColumns}
            loading={loading}
            onRowClick={(product) => router.visit(`/production/products/${product.id}`)}
            actions={(product) => (
                <EntityActionDropdown
                    onEdit={() => router.visit(`/production/products/${product.id}/edit`)}
                    onDelete={() => handleDelete(product)}
                    additionalActions={[
                        {
                            label: 'Gerenciar BOM',
                            icon: <Package className="h-4 w-4" />,
                            onClick: () => router.visit(`/production/products/${product.id}/bom`)
                        },
                        {
                            label: 'Histórico de BOM',
                            icon: <History className="h-4 w-4" />,
                            onClick: () => router.visit(`/production/products/${product.id}/bom-history`)
                        }
                    ]}
                />
            )}
...
const productColumns: ColumnConfig[] = [
    ...
    },
    {
        ...
        render: (value, product) => (
            product.current_bom ? (
                <Link 
                    href={`/production/bom/${product.current_bom_id}`}
                    className="text-primary hover:underline"
                >
                    {product.current_bom.bom_number}
                </Link>
            ) : (
...
#### 3.1.2 Product Details Page
**Route**: `/production/products/{id}`
**Layout**: `ShowLayout`
...
const tabs = [
    ...
    },
    {
        id: 'history',
        label: 'Histórico',
        content: <ProductHistoryTab product={product} />
    },
    {
        id: 'analytics',
        label: 'Análises',
...
function ProductOverviewTab({ product }) {
    return (
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                            <DetailItem label="Número" value={product.product_number} />
                        ...
                        <div className="space-y-4">
                            ...
                            <DetailItem label="Unidade de Medida" value={product.unit_of_measure} />
                            <DetailItem label="Lead Time" value={`${product.lead_time_days} dias`} />
                            <DetailItem label="Custo" value={formatCurrency(product.cost)} />
                        </div>
                    </div>
                </CardContent>
            </Card>
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between">
                        ...
                        <Button variant="outline" size="sm" asChild>
                            <Link href={`/production/bom/${product.current_bom_id}`}>
                    ...
                    <CardContent>
                        <BomSummary bom={product.current_bom} />
                    </CardContent>
                </Card>
            )}
        </div>
...
#### 3.1.3 Product Form Page
**Route**: `/production/products/create` or `/production/products/{id}/edit`
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Preencha as informações do produto de manufatura
                            </CardDescription>
                        </CardHeader>
                        ...
                        {mode !== 'view' && (
                            <CardFooter className="flex justify-end gap-4">
                                <Button variant="outline" type="button" asChild>
                                    <Link href="/production/products">Cancelar</Link>
                                </Button>
...
function BomTreeNode({ item, level, isExpanded, onToggle, onSelect, isSelected }) {
    return (
        <div
            className={cn(
