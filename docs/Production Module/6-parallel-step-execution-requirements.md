...
- **Immediate**: The next step can start as soon as the previous step begins processing its first unit
...
#### 2.3 Immediate Start Conditions

This rule allows the next step to start as soon as the previous step transitions to "in progress" status and begins processing its first unit. This enables the fastest possible flow through the production sequence, with each step beginning work as soon as any product becomes available from the previous step. This is particularly valuable for continuous processes or when minimizing total lead time is critical.
...
1. **Dependency Type Selector**: Radio buttons or dropdown to choose between "Wait for complete (100%)", "Start after quantity", "Start after percentage", or "Start immediately".
